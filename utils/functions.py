...
def get_pages(username):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM diary WHERE username=?' ,(username ,))
        results = cursor.fetchall()
        cursor.close()
        if len(results) == 0:
            return None
...
def add_pages(username,time,pages):

    conn = get_database_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO diary (username, time, pages) VALUES(?, ?, ?)",(username,time,pages) )
        conn.commit()
        cursor.close()
        return "Page added succefully"
    except:
        cursor.close()
	return "Insertion Failed Please check the query"
